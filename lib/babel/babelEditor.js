define(function (require) {
    ...
    var babelEditor = function () {
        ...
        var compileToES5 = function () {
            try {

                var input = editorES6.getSession().getDocument().getValue();
                var transformed;
        ...
        this.save = (function (_this) {
            return function (ext, editor) {
                ...
                _this.model.es6SourceStr(editorES6.getSession().getDocument().getValue())
                $.post("/saveModule", {
        ...
        this.update = (function (_this) {
            return function (updatedModel) {
                ...
                if (updatedModel.es6SourceStr().length > 0) {

                    editorES6_Session.getDocument().setValue(updatedModel.es6SourceStr());
                } else {
                    editorES5_Session.getDocument().setValue(updatedModel.es5SourceStr());
                    //  compileToCoffee();
                }
            };
        })(this);
        ...
        this.init = (function (_this) {
            return function (model) {
                ...
                editorES6_Session.setMode("ace/mode/coffee");
...
//                });
                ...
                _this.model = model;
                if (model && ItchCork.options.sourceExt === '.coffee') {
                    editorES6.getSession().getDocument().setValue(model.coffeeSourceStr().trim());
                    compileToJavaScript();
                } else {
                    editorES5.getSession().getDocument().setValue(model.jsSourceStr().trim());
                    compileToCoffee();
                }
            };
